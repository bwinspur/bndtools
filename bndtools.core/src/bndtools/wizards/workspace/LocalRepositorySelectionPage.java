...
class LocalRepositorySelectionPage extends WizardPage {
	...
	LocalRepositorySelectionPage(String pageName,
			RepositoryPlugin selectedRepository) {
		super(pageName);
	...
	public void createControl(Composite parent) {
		...
		Table table =
				new Table(parent, SWT.BORDER | SWT.SINGLE | SWT.FULL_SELECTION);
		...
		viewer.setFilters(new ViewerFilter[] { new ViewerFilter() {
			...
			public boolean select(Viewer viewer, Object parentElement,
				...
				return (element instanceof RepositoryPlugin)
						&& ((RepositoryPlugin) element).canWrite();
			}
		} });
		...
		viewer.addSelectionChangedListener(new ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent event) {
				...
				setSelectedRepository((RepositoryPlugin) selection
						.getFirstElement());
			}
		});
	...
	private void validate(Workspace workspace) {
		...
		List<RepositoryPlugin> plugins =
				workspace.getPlugins(RepositoryPlugin.class);
		if (plugins != null)
	...
	private void setSelectedRepository(RepositoryPlugin item) {
		...
		propSupport.firePropertyChange(PROP_SELECTED_REPO, old,
				selectedRepository);
